...
public class GameOfLife {
	...
	public static List<List<Cell>> advance(final List<List<Cell>> grid) {
		return grid.stream().map(list -> {
			return list.stream().map(cell -> {
	...
	private static int recursiveCountAliveNeighbours(final List<List<Cell>> grid, final int row, final int column, final int totalChecked, final int totalAlive) {
		if ((row != 0 && totalChecked == 0 && grid.get(row-1).get(column).isAlive() == 1) || 
		...
		return (List<List<Color>>) grid.stream()
				.map(list -> (List<Color>) list.stream()
						.map(cell -> {
							...
							final int numDarker = (neighbours == 8) ? 0 : (neighbours == 6 || neighbours == 7) ? 1 : (neighbours == 4 || neighbours == 5) ? 2 : (neighbours == 2 || neighbours == 3) ? 3 : 4;
							if (cell.isAlive() == 1) return IntStream.range(0, numDarker).mapToObj(el -> Color.white).reduce(Color.white, (acc, el) -> acc.darker());
